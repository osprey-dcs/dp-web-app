        ...
        <div className="flex flex-col items-center">
            <div className="w-full">
                {props.useRegex ? (
                    <input
                        ...
                        name="pv-regex"
                        value={props.dataSourcesRegex}
                        onFocus={() => setRegexPatternErrClass("")}
                        onChange={(e) =>
                            props.setDataSourcesRegex(e.target.value)
                        }
                        className={cn(
                            "input-std w-full mb-2",
                            regexPatternErrClass
                        )}
                    ></input>
                ) : (
                    <input
                        ...
                        name="pv-name"
                        value={props.dataSourcesString}
                        onFocus={() => setDataSourcesErrClass("")}
                        onChange={(e) =>
                            props.setDataSourcesString(e.target.value)
                        }
                        className={cn(
                            "input-std w-full mb-2",
                            dataSourcesErrClass
                ...
                <div className="flex items-center space-x-2">
                    <Switch
                        ...
                        onClick={() =>
                            props.setUseRegex((prevState) => !prevState)
                        }
                        onFocus={clearErrors}
                    />
            ...
            <button
                onClick={() =>
                    handleApply(
                        props.useRegex
                            ? props.dataSourcesRegex
                            : props.dataSourcesString
                    )
                }
                className="btn-std w-full mt-4"
            >
                Apply
            </button>
        </div>
